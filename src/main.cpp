...
//  Sudoku
//
//  Created by Étienne Collin on 2019-12-06.
//  Copyright © 2019 Étienne Collin. All rights reserved.
//
#include <iostream>
#include <vector>
#include <math.h>
...
bool isValidXY(double x, double y, int v) {
    int countX = 0; //Counts the number of time value 'v' is found in same row.
    int countY = 0; //Counts the number of time value 'v' is found in same column.
    
    //'i' is used to go through all the boxes.
    for (int i = 0; i < 9; i++) {
        //Verification for x and y axes.
        if(sudoku[i][y].value == v) countX++;
        if (sudoku[x][i].value == v) countY++;
    }
